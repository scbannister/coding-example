pilot_cleaning <- function(x){
  ...
  ## Function takes dataset as input, and is written to clean up any messy aspects of pilot dataset
  
  ## Function achieves the following: 
  
  ## Improve consistency in "nationality" variable responses
  ## Applies descriptive labels to values in "musicianship", "gender", "training" and "listening" variables
  ## Re-scores trait characteristics measures where needed
  ## Creates composite scores for these trait characteristics
  ## Restructures emotion data from wide to long format
  ## Recodes emotion data
  ## Finally calculates emotion dimension scores from the emotion data
  
  ###################################################
  # Tidy up "nationality" variable
  ###################################################
  
  x$Nationality[x$Nationality == "English"] <- "British"  # This code (and repeated commands below)
  #takes any value in variable that matches the value within square brackets, and replaces it with the 
  # new value on the right side of the assignment arrow
  x$Nationality[x$Nationality == "White British"] <- "British"
  x$Nationality[x$Nationality == "South Korean"] <- "South Korean / Korean"
  x$Nationality[x$Nationality == "Korean"] <- "South Korean / Korean"
  x$Nationality[x$Nationality == "Korean(South)"] <- "South Korean / Korean"
  x$Nationality[x$Nationality == "tangata tiriti"] <- "Tangata Tiriti"
  
  ###################################################
  # Descriptive labels for demographics data
  ###################################################
  
  # Musicianship
  x$Musicianship[x$Musicianship == 1] <- "Not Music Professional or Student"
  
  # Gender
  x$Gender[x$Gender == 1] <- "Male"
  x$Gender[x$Gender == 2] <- "Female"
  x$Gender[x$Gender == 5] <- "Gender Variant / Non-Conforming"
  
  # Musical Training
  x$Training[x$Training == 1] <- "No Musical Training"
  x$Training[x$Training == 2] <- "Few Years of Musical Training"
  x$Training[x$Training == 3] <- "Can Play Instrument But No Training"
  
  # Listening
  x$Listening[x$Listening == 1] <- "At Least once a Day"
  x$Listening[x$Listening == 2] <- "Once or Twice a Week"
  x$Listening[x$Listening == 3] <- "Less Than Once or Twice a Week"
  ...
  for (k in 1:length(participants)) {
    ...
    # Function then returns tidy original dataset, and the full and trimmed datasets for further analysis
  return(list(x, df_full, df_trim))
