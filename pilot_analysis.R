...
# Import dataset
dataset <- read_csv('replication_pilot_data.csv', col_names = TRUE)

# Remove incomplete and ineligible responses (i.e., those who did not fully complete, or those who are a music student or music professional)
dataset <- dataset %>%
  filter(!(Progress < 100)) %>%
  filter(!(Musicianship == 3))

# Run data cleaning function and change data to long format and prepare for analysis; function outputs three datasets within a "df" list object
df <- pilot_cleaning(dataset)

dataset <- as.data.frame(df[1]) # Cleaned up original dataset
df_full <- as.data.frame(df[2]) # Here we can extract the full long dataset
df_trim <- as.data.frame(df[3]) # Here we can extract the trimmed dataset
rm(df) # We no longer need the original dataset or list object from function
...
model <- anova_test(as.data.frame(test_data), dv = M, wid = participant, within = c(task, condition), covariate = c(anxiety),  effect.size = 'ges', detailed = TRUE)
get_anova_table(model, correction = 'auto') %>%
  adjust_pvalue(method = 'bonferroni')

model <- anova_test(as.data.frame(test_data), dv = M, wid = participant, within = c(task, condition), covariate = c(sensation),  effect.size = 'ges', detailed = TRUE)
get_anova_table(model, correction = 'auto') %>%
  adjust_pvalue(method = 'bonferroni')

model <- anova_test(as.data.frame(test_data), dv = M, wid = participant, within = c(task, condition), covariate = c(BIS),  effect.size = 'ges', detailed = TRUE)
tab <- get_anova_table(model, correction = 'auto') %>%
  adjust_pvalue(method = 'bonferroni')

model <- anova_test(as.data.frame(test_data), dv = M, wid = participant, within = c(task, condition), covariate = c(BAS),  effect.size = 'ges', detailed = TRUE)
get_anova_table(model, correction = 'auto') %>%
  adjust_pvalue(method = 'bonferroni')

model <- anova_test(as.data.frame(test_data), dv = M, wid = participant, within = c(task, condition), covariate = c(IRI),  effect.size = 'ges', detailed = TRUE)
get_anova_table(model, correction = 'auto') %>%
  adjust_pvalue(method = 'bonferroni')
